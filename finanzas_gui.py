...
class CalculadoraFinanciera:
    ...
    def mostrar_campos(self):
        ...
        elif oper == "Pagos / Anualidad (FV)":
            self.crear_campos(["FV", "Tasa (%)", "Nº periodos"])
            ttk.Button(self.frame_campos, text="Calcular", command=self.calc_pmtFv).pack(pady=5)
        ...
        elif oper == "Valor Futuro de una Anualidad":
            self.crear_campos(["PMT", "Tasa (%)", "Nº periodos"])
            ttk.Button(self.frame_campos, text="Calcular", command=self.calc_vfA).pack(pady=5)
